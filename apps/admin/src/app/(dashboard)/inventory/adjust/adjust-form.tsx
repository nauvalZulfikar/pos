...
export function AdjustForm({
    ...
    return (
      <div className="rounded-md border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
        {outlets.length === 0
          ...
          : 'Belum ada bahan baku. Tambahkan dulu di /inventory/new.'}
      </div>
    );
  }
  ...
  return (
    <form onSubmit={onSubmit} className="max-w-lg space-y-4 rounded-xl border border-slate-200 bg-white p-6">
      ...
      <Field label="Delta (+ tambah, − kurang)">
        <input
          name="delta"
          type="number"
          step="any"
        ...
        <p className="mt-1 text-xs text-slate-500">
          Dalam unit dasar bahan (gram/ml/pcs). Negatif untuk pengurangan.
        </p>
      </Field>
      ...
      <Field label="Alasan (wajib)">
        <input
          ...
          minLength={3}
          maxLength={200}
          placeholder="contoh: stock opname, restock, expired"
          className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
        />
      </Field>

      {state.status === 'ok' ? (
        <p className="text-sm text-emerald-700">✓ Stok berhasil disesuaikan.</p>
      ) : null}
      ...
      >
        {state.status === 'busy' ? 'Menyimpan...' : 'Simpan'}
      </button>
    </form>
  );
}
